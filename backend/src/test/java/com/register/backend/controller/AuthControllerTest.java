...
// This slice only loads AuthController, not the app's SecurityConfig. addFilters = false disables the
// security filter chain since this test isn't exercising security at all — POST /api/auth/login is
// genuinely public and the JWT filter chain is verified end to end in SecurityIntegrationTest instead.
...
class AuthControllerTest {
    ...
    void loginReturnsTokenWhenCredentialsAreValid() throws Exception {
        LoginRequest request = new LoginRequest("admin", "correct-password");
        LoginResponse response = new LoginResponse("signed-jwt-token", "admin", "ROLE_ADMIN");
    ...
    @Test
    void loginReturns400WhenUsernameIsBlank() throws Exception {
        LoginRequest request = new LoginRequest("", "some-password");
